...
我们可以用有序对 $$ (x_i)_{1\leq i\leq n} $$ 表示函数 $i\mapsto x_i$，于是我们说明了 $n$ 重 Cartesian 积确实是一个集合。
...
$$
    g(y):=\begin{cases}f(y)&f(y)<f(x),\\[3pt]g(y)\;\text{satisfies}\;g(y)++=f(y)&f(y)>f(x).\end{cases}
$$

下面证明 $g$ 也是一个双射：$g$ 是一个单射，因为