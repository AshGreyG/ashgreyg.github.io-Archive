...
- $f:X\mapsto\emptyset$，按照上面的讨论 $f$ 是一个满射而不是单射（我也查到有说法这样的映射 $f$ 是不存在的，而且也可以根据命题 1.2-10-f 可以得到这样的结论）。
...
这样就存在一个自然数 $M+1$ 与所有 $f(i)$ 不相等，与 $f$ 是一个双射的前提假设矛盾，于是可以推出 $\mathbb{N}$ 是一个无限集。
...
根据命题 1.2-10-b 可知 $X\times Y'$ 是一个有限集，并且
...
证明：不妨令 $$ \#(X)=m,\#(Y)=n $$，我们对 $n$ 进行归纳。当 $n=0$ 时，$Y=\emptyset$，则根据幂集的定义：$$ Y^X=\{f:\;f:X\mapsto Y\} $$，只存在一个