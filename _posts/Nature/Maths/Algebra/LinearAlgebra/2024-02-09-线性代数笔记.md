...
> **命题 3.4-4 $\mathcal{L(V,W)}$ 与 $\mathbb{F}^{m\times n}$ 同构**
>
> 设 $\boldsymbol{v_1},\cdots,\boldsymbol{v_n}$ 是 $\mathcal{V}$ 的基，$\boldsymbol{w_1},\cdots,\boldsymbol{w_n}$ 是 $\mathcal{W}$ 的基，则 $\mathcal{M}$ 是 $\mathcal{L(V,W)}$ 和 $\mathbb{F}^{m\times n}$ 之间的同构。即我们有 $\dim\mathcal{L(V,W)}=(\dim\mathcal{V})(\dim\mathcal{W})$
...
- 满射：考虑 $\mathbf{A}\in\mathbb{F}^{m\times n}$，令 $\mathscr{T}:\mathcal{V\mapsto W}$ 且定义为

    $$
        \mathscr{T}(\boldsymbol{v_k})=\sum_{j=1}^mA_{j,k}\boldsymbol{w_j},\; k=1,\cdots,n
    $$

    则 $\mathcal{M}(\mathscr{T})=\mathbf{A}$，因此 $\text{range}\mathcal{M}=\mathbb{F}^{m\times n}$，故 $\mathcal{M}$ 是一个满射。

综上所述，$\mathcal{M}$ 是一个可逆的线性映射，并且是 $\mathcal{L(V,W)}$ 与 $\mathbb{F}^{m\times n}$ 之间的同构。
