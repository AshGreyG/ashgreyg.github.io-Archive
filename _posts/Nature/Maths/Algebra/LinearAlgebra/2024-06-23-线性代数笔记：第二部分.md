...
很浅显的，所有的自伴算子都是正规的。我们说明正规算子的性质是满足齐次性和可加性的：
- 可加性：设正规算子 $\mathscr{S},\mathscr{T}\in\mathcal{L}(\mathcal{V})$，我们说明 $\mathscr{S}+\mathscr{T}$ 是正规的：

    $$
        \begin{aligned}
            LHS&=(\mathscr{S}+\mathscr{T})\circ(\mathscr{S}+\mathscr{T})^*\\
            &=(\mathscr{S}+\mathscr{T})\circ(\mathscr{S}^*+\mathscr{T}^*)\\
            &=\mathscr{S}\circ(\mathscr{S}^*+\mathscr{T}^*)+\mathscr{T}\circ(\mathscr{S}^*+\mathscr{T}^*)\\
            &=
        \end{aligned}
    $$
...
证明：因为 $\mathscr{T}$ 是正规的，所以 $\mathscr{T}-\lambda\mathscr{I}$ 也是正规的